...
class AIPlayer(Player):
    # Learning constants
    ALPHA = 0.1  # Learning rate (how much to update values based on new information)
    GAMMA = 0.9  # Discount factor (how much to value future rewards vs immediate rewards)
    ...
    def hashState(self, currentState):
        ...
        if carrying_worker is not None:
            ...
            if len(deposit_coords) > 0:
                ...
                for dep_coords in deposit_coords:
                    dist = stepsToReach(currentState, worker_coords, dep_coords)
                    if dist >= 0:
        ...
        if non_carrying_worker is not None:
            ...
            if len(foods) > 0:
                first_food = foods[0]
                dist = stepsToReach(currentState, worker_coords, first_food.coords)
                if dist >= 0:
        ...
        if len(my_attackers_list) > 0:
            ...
            if enemy_queen is not None:
                dist = stepsToReach(currentState, attacker_coords, enemy_queen.coords)
                if dist >= 0:
        ...
        if len(my_attackers_list) > 0:
            ...
            if enemy_hill is not None:
                dist = stepsToReach(currentState, attacker_coords, enemy_hill.coords)
                if dist >= 0:
